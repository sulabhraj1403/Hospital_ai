...
async function makeVideo(
  ...
  if (!FFmpegNS?.FFmpeg) {
    throw new Error(
      "FFmpeg library failed to load. Please refresh the page."
    );
  }

  if (
    !FFmpegUtilNS?.fetchFile ||
    !FFmpegUtilNS?.toBlobURL
  ) {
    throw new Error(
      "FFmpeg utility library failed to load. Please refresh the page."
    );
  }
...
$("generate").onclick = async () => {
  ...
  try {
    ...
    status(
      "Creating script and scene plan…",
      3
    );
    ...
    if ($("music").checked) {
      ...
      status(
        "Finding suitable background music…",
        63
      );

      musicInfo =
        await getMusic(topic);
